...
import { Bell, Check, X, Edit3, Trash2, Loader2, KeyRound, Eye, EyeOff } from "lucide-react";
...
export default function NotifikasiBell() {
  const { pending, pendingPasswords, count, loading, approve, reject, isApprover, isPasswordApprover } = useApprovals();
  ...
  // Close on click outside (termasuk klik di backdrop portal)
  useEffect(() => {
    if (!open) return;
                    ...
                    <>
                      ...
                      {pendingPasswords.map((req) => (
                        <div key={req.id} className="px-4 py-3 hover:bg-gray-50/50 transition-colors">
                          ...
                          {/* Password baru (bisa dilihat) */}
                          <div className="bg-gray-50 rounded-lg p-2 mb-2 space-y-1">
                            <div className="flex items-center justify-between text-[11px]">
                              <span className="text-gray-500">Password baru:</span>
                              <button
                                onClick={() => setShowNewPwd((prev) => ({ ...prev, [req.id]: !prev[req.id] }))}
                                className="p-0.5 rounded text-gray-400 hover:text-gray-600 transition-all"
                              >
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => handleApprove(req.id, 'password')}
                              disabled={processing === `approve-${req.id}`}
                              className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-all disabled:opacity-50"
                            >
                              {processing === `approve-${req.id}` ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
                              Setujui
                            </button>
                            <button
                              onClick={() => handleReject(req.id, 'password')}
                              disabled={processing === `reject-${req.id}`}
                              className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100 transition-all disabled:opacity-50"
                            >
                              <X className="w-3 h-3" />
                              Tolak
                            </button>
