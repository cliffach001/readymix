...
export default function LoginScreen() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      login(result.data.username, result.data.role as any, result.data.nama_lengkap, result.data.unit_kerja ?? undefined, result.data.avatar_url || undefined);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gradient-to-br from-orange-600 via-orange-700 to-red-800">
      ...
      <div className="relative z-10 w-full max-w-md px-4">
        <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-5 sm:p-8 md:p-10 shadow-2xl border border-white/20 animate-[slideUp_0.5s_ease-out]">
          ...
          <div className="text-center mb-6 sm:mb-8">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Penjualan Ready Mix</h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1">PT. Prima Karya Manunggal</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-[11px] sm:text-xs">
              <label className="flex items-center gap-1.5 text-gray-500 cursor-pointer">
                <input type="checkbox" defaultChecked className="accent-[#F35b04] w-3 h-3 sm:w-3.5 sm:h-3.5" />
                Ingat saya
              </label>
              <button type="button" onClick={() => alert("Hubungi admin untuk reset password")} className="text-[#F35b04] font-medium hover:underline whitespace-nowrap ml-2">
                Lupa password?
              </button>
            </div>
